...
export function ArtifactBuild() {
  ...
  useEffect(() => {
    ...
    const sync = () => setTight(!query.matches);
    sync();
    query.addEventListener("change", sync);
    return () => query.removeEventListener("change", sync);
  }, []);

  const held = still || tight;

  return (
    <motion.div
      initial={false}
      animate={{ width: held ? "100%" : ["60%", "60%", "100%", "100%"] }}
      transition={{
        duration: 7,
        times: [0, 0.35, 0.65, 1],
    ...
    >
      ...
      <div className="p-4">
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {[0, 1, 2].map((card) => (
            <div key={card} className="min-w-24 flex-1 rounded-md border p-2.5">
