...
export function NorviaHome() {
  ...
  return (
    <div className="concept-page concept-theme-paper bg-concept-canvas text-concept-ink font-display absolute top-0 left-0 flex flex-col">
      <header
        style={cue(0)}
        className="stage-cue build-part border-concept-ink/10 flex h-12 shrink-0 items-center gap-8 border-b px-8"
      >
        <span className="font-label text-[0.9em] tracking-[0.32em]">NORVIA</span>
        <nav className="flex gap-6 text-[0.72em]">
          {links.map(link => (
            <span key={link}>{link}</span>
          ))}
        </nav>
        <span className="text-concept-muted font-label ml-auto text-[0.55em] tracking-[0.18em]">
        ...
        <button
          ...
          className={cn(
            "font-label flex items-center gap-2.5 border px-3.5 py-1.5 text-[0.6em] tracking-[0.2em] transition-colors duration-300",
            count
      ...
      <div className="grid h-64 shrink-0 grid-cols-2">
        <div className="flex flex-col justify-center px-8">
          <span
            style={cue(0.06)}
            className="stage-cue build-part text-concept-muted font-label text-[0.58em] tracking-[0.2em]"
          >
            HUDSON, NEW YORK · SINCE 2019
          </span>
          <p
            style={cue(0.1)}
            className="stage-cue build-part font-concept-display mt-2.5 max-w-[16ch] text-[2.2em] leading-[0.95]"
          >
            Thrown by hand,{" "}
          ...
          <p
            style={cue(0.14)}
            className="stage-cue build-part text-concept-muted mt-2.5 max-w-[44ch] text-[0.8em] leading-relaxed"
          >
          ...
          <div style={cue(0.18)} className="stage-cue build-part mt-4 flex gap-3">
            <button
              type="button"
              className="bg-concept-clay text-concept-canvas font-label px-5 py-2.5 text-[0.6em] tracking-[0.2em] transition-opacity duration-300 hover:opacity-85"
            >
              SHOP THE DROP
            </button>
            <button
              ...
              className="border-concept-ink/25 hover:bg-concept-shell font-label border px-5 py-2.5 text-[0.6em] tracking-[0.2em] transition-colors duration-300"
            >
              BOOK A CLASS
            </button>
          </div>
        ...
        >
          ...
          />
          <span className="bg-concept-canvas/90 font-label absolute bottom-3 left-3 px-2 py-1 text-[0.5em] tracking-[0.18em]">
            MARA, AT THE WHEEL
          </span>
        </div>
      </div>
      ...
      <div className="border-concept-ink/10 divide-concept-ink/10 grid min-h-0 flex-1 grid-cols-12 divide-x border-t">
        <div className="col-span-5 flex min-h-0 flex-col p-5">
          <span
            style={cue(0.3)}
            className="stage-cue build-part text-concept-clay font-label text-[0.58em] tracking-[0.2em]"
          >
              ...
              return (
                ...
                >
                  <span className="bg-concept-shell relative min-h-0 w-full flex-1 overflow-hidden">
                    ...
                    {tag ? (
                      <span className="build-act bg-concept-canvas/90 font-label absolute top-1.5 left-1.5 px-1.5 py-0.5 text-[0.5em] tracking-[0.16em]">
                        {tag}
                      </span>
                    ) : null}
                    {!gone ? (
                      <span className="bg-concept-ink text-concept-canvas font-label absolute right-1.5 bottom-1.5 px-1.5 py-0.5 text-[0.5em] tracking-[0.16em] opacity-0 transition-opacity duration-300 group-hover/good:opacity-100">
                        + ADD
                      </span>
                    ) : null}
                  </span>
                  <span className="mt-2 flex shrink-0 items-baseline justify-between gap-2 text-[0.72em]">
        ...
        <div className="col-span-4 flex min-h-0 flex-col p-5">
          <span
            style={cue(0.48)}
            className="stage-cue build-part text-concept-clay font-label text-[0.58em] tracking-[0.2em]"
          >
            ...
            {sessions.map((option, index) => {
              ...
              return (
                <div
                  ...
                  className="stage-cue build-part flex flex-1 items-center gap-3 text-[0.72em]"
                >
                  ...
                  <button
                    ...
                    className="border-concept-ink/25 hover:bg-concept-shell font-label ml-auto shrink-0 border px-2.5 py-1 text-[0.7em] tracking-[0.16em] transition-colors duration-300 disabled:opacity-40"
                  >
                    {left ? "BOOK" : "FULL"}
                  </button>
                </div>
        ...
        >
          ...
          <span className="text-concept-chalk absolute inset-x-5 top-4 flex flex-col gap-1">
            <span className="font-label text-[0.55em] tracking-[0.2em] opacity-70">
              COMMISSIONS · FROM $480
            </span>
            <span className="font-concept-display flex items-center gap-2 text-[1.15em] leading-tight">
      ...
      <footer
        style={cue(0.66)}
        className="stage-cue build-part border-concept-ink/10 flex h-13 shrink-0 items-center gap-5 border-t px-8 text-[0.72em]"
      >
        <span className="text-concept-muted font-label text-[0.8em] tracking-[0.2em]">
          FIND US IN PERSON
        </span>
        <span>
